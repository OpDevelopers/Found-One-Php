...
function filterData($data, $location, $purpose, $price) {

    // Filter the data based on user input
    $filteredData = array_filter($data, function ($item) use ($location, $purpose, $price) {
        $capitalizedLocation = ucwords($location);

        // Filter by location, purpose, and price
        $locationMatch = empty($location) || ucwords($item->address) === $capitalizedLocation;
        $purposeMatch = empty($purpose) || $item->tag === $purpose;
        $priceMatch = empty($price) || $item->price === $price;

        // Determine if this section matches any of the search criteria
        if($locationMatch){
          return true;
          ...
          return true;
        }else if(($locationMatch && $priceMatch) || ($locationMatch && $purposeMatch) || ($priceMatch && $purposeMatch) || ($locationMatch && $priceMatch && $purposeMatch)){
          return true;
        }
    });
...
// Example usage:
$userLocation = isset($_POST['location']) ? $_POST['location'] : '';
$userPurpose = isset($_POST['purpose']) ? $_POST['purpose'] : '';
$userPrice = isset($_POST['price']) ? $_POST['price'] : '';

$filteredData = filterData($filePath, $userLocation, $userPurpose, $userPrice);
echo $userLocation;
echo $userPurpose;
echo $userPrice;
...
<!-- Your HTML code here to display the filtered data -->
  ...
  <div class="row row-cols-2 g-1">
    <div class="col-lg-3 filter-search">
        <div class="container">
            ...
            <!--Search Form Element -->
              <form action="/filterSearch" method="POST" class="fs-3">
                <select class="form-select py-2 my-2" aria-label="Default select example" name="location">
                <?php foreach($filterPlacePath[0]->places as $locations){?>
    ...
    <div class="album col-lg-8 py-5 bg-body-ternary m-auto medium-device">
        <div class="container">
          <div class="row col-lg-12 my-2">
            ...
            <h5> Total <?php echo count($filteredData) ?></h5>
            </div>
          </div>
...
<div class="links">
  <a href="allListing">1</a>
  <a href="allListing2">2</a>
  <a href="allListing3">3</a>
  <a href="allListing4">4</a>
  <a href="allListing5">5</a>
  <a href="allListing6">6</a>
  <a href="allListing7">7</a>
</div>
